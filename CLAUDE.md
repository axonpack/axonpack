...
- `bun run lint:oxlint` — `oxlint src`, an oxlint-based alternative to the eslint gate above. Not wired into `pre-commit` yet — its rules come from `oxlint.config.mts` extending the shared `linter` base config (`packages/linter`).
