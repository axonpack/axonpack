...
- Root `workspaces` glob is non-standard because of the `@axonpack` npm scope directory and a nested example app:
  `packages/*`, `packages/@axonpack/*`, `packages/@axonpack/expo-devtools/example`. There is deliberately **no `apps/*`** and no `docs` entry, because `docs/` is a submodule with its own install, and listing it would make bun hoist its dependencies to this root, which breaks its Turbopack root and stops it building on its own.
...
- **`bun run lint` currently reports 4 warnings**, all from React rules the old base did not carry: three `react(set-state-in-effect)` and one `react(immutability)`, in the network detail panel and the crash stack section. They are real observations about async components, not noise. They are left visible rather than disabled or blind-fixed. Warnings do not fail the gate.
