...
- Root `workspaces` glob is non-standard because of the `@axonpack` npm scope directory and a nested example app:
  `packages/*`, `packages/@axonpack/*`, `packages/@axonpack/expo-devtools/example`. There is deliberately **no `apps/*`** and no `docs` entry — `docs/` is a submodule with its own install, and listing it would make bun hoist its dependencies to this root, which breaks its Turbopack root and stops it building on its own.
