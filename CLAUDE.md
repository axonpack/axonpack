...
Turborepo + bun workspaces monorepo intended to hold `@axonpack/*` — free OSS foundation libraries for React Native/Expo (`@axonpack/lite-storage`, `@axonpack/expo-devtools`, `@axonpack/api-kit`, `@axonpack/i18n`). **Two are implemented: `@axonpack/expo-devtools` and `@axonpack/react-pretty-print`.** `docs/` is the documentation site — Next.js + Fumadocs, MDX under `content/docs/<package-slug>/` — but it is **not part of this repo**: it is a git submodule of `axonpack/docs`, mounted the same way `marketing/` is. It is self-contained (own lockfile, own `node_modules`, own oxlint config, no workspace dependency) and it builds and deploys itself; nothing here builds or lints it. `packages/linter` (npm name: `linter`, deliberately _not_ `@axonpack/*`-scoped) is separate from that roadmap — it's an internal, non-public shared oxlint base config, not a `@axonpack/*` product library. Only packages actually meant for npm carry the `@axonpack/` scope; right now that's `@axonpack/expo-devtools` and `@axonpack/react-pretty-print`.

`README.md` is the public front page and is current — logo, screenshots, the **Packages** table with an npm version badge per row, the install and script list. Keep the table in step with what's under `packages/@axonpack/`.
...
- Root `workspaces` glob is non-standard because of the `@axonpack` npm scope directory and a nested example app:
  `packages/*`, `packages/@axonpack/*`, `packages/@axonpack/expo-devtools/example`, `packages/@axonpack/react-pretty-print/example-web`, `packages/@axonpack/react-pretty-print/example-native`. There is deliberately **no `apps/*`** and no `docs` entry — `docs/` is a submodule with its own install, and listing it would make bun hoist its dependencies to this root, which breaks its Turbopack root and stops it building on its own.
  A plain `packages/*` glob does **not** match `packages/@axonpack/expo-devtools` (two levels deep) — new scoped packages are covered by the `packages/@axonpack/*` entry, but each package's own example app needs its own explicit workspace entry to get linked via bun (otherwise its `@axonpack/<pkg>` dependency won't resolve to the local package). `@axonpack/react-pretty-print` has two, hence two entries.
...
### `@axonpack/react-pretty-print` package (run from `packages/@axonpack/react-pretty-print`)

- `bun run build` — `rm -rf build && tsc`. It **does** clean first, unlike the sibling package: `tsc` alone leaves output behind for sources that were renamed or deleted, and that output gets published.
- `bun run lint` / `bun run format` — `oxlint src` / `oxlint src --fix`, same shared `linter` base. Currently **0 warnings**.
- `bun run check-types` — `tsc --noEmit -p tsconfig.check.json`. Two configs on purpose: `tsconfig.json` drives the build and must exclude `*.test.ts*` or the tests land in `build/`; `tsconfig.check.json` clears that exclude so the tests are typechecked too. Without it, renaming a public type breaks the tests only at runtime.
- `bun run test` — `bun test src`. No jest: the package has no native code and bun's runner needs no config. ~39 tests. `react-dom` is a devDependency purely so the tests can `renderToStaticMarkup` the DOM path.
...
- `commit-msg` — runs commitlint (`commitlint.config.js` at repo root): conventional-commit `type` restricted to a fixed enum, and if a scope is given it must be one of `@axonpack/expo-devtools`, `@axonpack/react-pretty-print`, `linter` or `docs` — extend `scope-enum` there when adding a package. Commits touching a package's example app conventionally take no scope.
...
Three independent interception paths feed one shared store (`features/network/stores/network-log.store.ts` — in-memory ring buffer capped at 200 entries, pub/sub via `expo`'s `EventEmitter`, read via `useSyncExternalStore` in `network-view.component.tsx`):

- `features/network/services/patch-fetch.service.ts` — wraps `globalThis.fetch`. Required because **Expo installs its own native fetch by default** (`expo/winter/fetch`), which does not route through `XMLHttpRequest` the way the old whatwg-fetch polyfill did — patching XHR alone cannot see it.
...
Nothing patches the registered store instance, so there is no mutation history and app behaviour is unchanged — the tab only calls what it was handed. `write-storage.service.ts` returns an error string rather than throwing, and re-reads the one key it touched instead of trusting what it wrote, since a store may normalise the value. An entry's display type (`StoredValueKind`) is classified **once at read time** and stored on the entry, because `classifyStoredValue` parses JSON and re-running it for a thousand keys on every filter keystroke would be the slowest thing in the tab. MMKV has no type query, so the type is probed `getString` → `getNumber` → `getBoolean` → `getBuffer`, each checked `!== undefined` and never for truthiness — a stored `0` or `false` is a value, and treating it as a miss would hide the key entirely. `StorageView` reads on mount rather than at `init()`: a store the panel is never opened on shouldn't be read at all. It holds the **selected key**, not the selected entry, because an edit replaces the entry object in the store and a sheet holding the old one would keep showing the value you just changed.

### Example app (`example/`)

Regenerated via `create-expo-app` (not the `create-expo-module` template) — hence its own `assets/`/`app.json` rather than the module scaffold's. `App.tsx` is just a top-level tab switcher; real screens live in `components/`: `NativeRequests.tsx` (fetch/XHR/third-party HTTP client demo buttons), `WebViewDemo.tsx` (loads a real external site so its traffic gets captured), `RequestsScreen.tsx` (Native/WebView sub-tabs), `TabBar.tsx` (shared, `primary`/`secondary` visual variants), `StorageDemo.tsx` (seeds AsyncStorage / MMKV / SecureStore / an in-memory `Map` with a spread of value shapes, and can write past the 1,000-key read cap). `devtools.ts` creates the one shared client instance — and owns the MMKV instance and the in-memory `Map` the storage adapters are registered with — imported by `index.ts` (`.init()`), `WebViewDemo.tsx` (`.getWebViewInjectedJavaScriptBeforeContentLoaded`/`.handleWebViewMessage`) and `StorageDemo.tsx`. The example is the only place the three real storage libraries are installed; it typechecks against them, which is what keeps the duck types in `define-adapter.service.ts` honest. AsyncStorage and SecureStore are inside Expo Go, so they work under `bun run start`; MMKV is not, and `createMMKV()` **throws** when its native module is missing — so `devtools.ts` wraps it in a try/catch, exports `mmkv` as possibly-null, and spreads the MMKV adapter in conditionally. Without that the example crashes at import in Expo Go rather than degrading. Adding native deps means a prebuild and a native rebuild (`bun run ios`), not just `bun install`.

## Architecture: `@axonpack/react-pretty-print`

Pure JS/TSX, no native code, and **no platform import anywhere in `src`** — that is the whole design, not an incidental property. Three renderers (`JsonTree`, `XmlTree`, `CodeHighlight`) serve React and React Native from one implementation because the caller passes the components in:

```tsx
<JsonTree primitives={{ View, Text, Pressable }} value={data} />   // react-native, no adapter
<JsonTree primitives={domPrimitives} value={data} />                // the DOM, shipped
```

- **`Primitives` (`src/components/shared/primitives.ui.tsx`).** `View`, `Text`, `Pressable`, typed loosely because RN's `StyleProp<ViewStyle>` and React's `CSSProperties` are not assignable to each other in either direction. react-native's own components satisfy the contract **as-is**, which is deliberate and load-bearing: the long-press event is typed `unknown` and passed through untouched, because narrowing it (`never` included) fails contravariance and would force every RN consumer to write a wrapper. `domPrimitives` ships for the web, since the DOM has no `onPress`.
- **The context menu is not in the package**, and that is a boundary, not an omission. `onRequestMenu(items, event)` hands back the items that apply to a node — which ones, and what each does to expansion state, is the shareable part — and the caller renders the popover. A floating menu has to escape its scroll container, which needs `Modal` + `useWindowDimensions` on RN and a portal + `position: fixed` on the DOM; RN has no portal and the DOM has no Modal. Clipboard is the caller's for the same reason (`onCopy`). Both examples' `components/ContextMenu.tsx` are the reference implementations.
- **Styling constraint.** RN and CSS disagree on three _defaults_, not on property names, and all three are stated explicitly in `utils/shared/tree-styles.util.ts`: `display` (RN is always `flex`, a `div` is `block`, so `flexDirection` is inert until stated), `flexDirection` (RN `column`, CSS `row`) and `flexShrink` (RN `0`, CSS `1`). Longhand only — RN understands `paddingVertical` and CSS doesn't. `flex: 1` belongs only on a row's single filling node; on siblings each grows to an equal share. The one deliberately web-only key is `whiteSpace: 'pre'` on the code block: `formatCode` emits newlines that HTML collapses, and RN has no such style prop and no longer validates inline styles, so it drops it.
- **Layout** is layer-first, domain-second — `components/`, `utils/` each holding `json/ xml/ code/ shared/`, plus `themes/` beside them. Every folder has an `index.ts`; the per-layer ones are mechanical, and `src/index.ts` is curated because those barrels expose the internal `JsonNode`/`XmlNode` renderers and the style builders. It cannot be a starred re-export of both `./components` and `./utils`: `XmlNode` is a renderer in one and the parsed-node type in the other. `src/index.test.ts` pins the published surface so a barrel can't leak or drop an export unnoticed.
- **Two entry points.** `.` for the renderers and the pure logic; `./themes` for the palettes and `PrettyPrintTheme`. `palettes.const.ts` is ~52KB, so starring it from the root barrel meant a consumer who only wanted a renderer pulled all 130 into the module graph to discover they didn't need them.
- **Themes** are a token-role map, never a stylesheet, so there is nothing to translate per platform. 130 palettes: two achromatic hand-tuned ones the renderers default to (no other palette is neutral — every constructed one is tinted toward its base hue), plus 128 built from a base hue, a mode and one of four characters. The construction rules live in `palettes.const.ts`'s header, because the file is ordinary source: **edit a colour if you want to.** The guarantee is in `themes.test.ts`, not in how the file was written — every colour clears a WCAG ratio against its own background (4.5:1 body text, 7:1 on `crisp`, never under 3:1), keys never match strings, and a hue-named palette really is that hue. Contrast is hue-blind, so that last one catches a channel rotation the ratio assertions cannot.
- **The tokenizer** (`utils/code/`) is a longest-prefix-wins rule list over **38 languages** with no dependencies, grouped into lexical families — fifteen of them differ only in their keyword vocabulary, which is why `braceFamily()` takes a list and `languages.const.ts` holds the words. `detectLanguage` reads a content type and falls back to sniffing the body, which is the common case. `formatCode` re-indents only the brace-and-semicolon languages plus json and the stylesheets; python and yaml carry meaning in their existing indentation, and the exclusion is asserted rather than just documented. Adopting highlight.js or Prism for their ~200–300 languages was considered and rejected: it brings a CSS-shaped theme format that has to be translated to RN styles at runtime, which is where that cost lives.
- **`XmlTree`** parses in JS because React Native has no `DOMParser`, and a malformed document returns an error for the caller to show its raw text against rather than throwing. It re-parses when `source` changes — holding the first parse in state only looks correct while the caller happens to remount.

## Known quirks worth remembering

- Root `turbo.json`'s generic `outputs: [".next/**", ...]` came from the Next.js starter. Nothing in this repo builds a Next app any more — the docs submodule has its own — and it doesn't match `@axonpack/expo-devtools`'s actual build output (`build/**`), so `turbo run build` warns `no output files found for task @axonpack/expo-devtools#build` and can never cache that one. Not a bug to "fix" reflexively; just don't be surprised that package's build always re-runs.
  `@axonpack/react-pretty-print` **does** declare its own `outputs`, in a package-scoped task entry, and that is load-bearing rather than tidiness: `pretty-print-example-web#check-types` depends on that build, and a cache hit with no recorded outputs replays the logs without restoring `build/`, so the example's typecheck then fails on a clean checkout.
