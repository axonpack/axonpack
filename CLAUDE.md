...
Turborepo + bun workspaces monorepo intended to hold `@axonpack/*` — free OSS foundation libraries for React Native/Expo (`@axonpack/lite-storage`, `@axonpack/expo-devtools`, `@axonpack/api-kit`, `@axonpack/i18n`). **Only `@axonpack/expo-devtools` is implemented so far**; `apps/` holds one app, `apps/docs` (npm name: `axonpack-docs`, private) — the documentation site, Next.js + Fumadocs, statically exported to GitHub Pages, whose pages are MDX under `apps/docs/content/docs/<package-slug>/`. `packages/linter` (npm name: `linter`, deliberately _not_ `@axonpack/*`-scoped) is separate from that roadmap — it's an internal, non-public shared oxlint base config, not a `@axonpack/*` product library. Only packages actually meant for npm carry the `@axonpack/` scope; right now that's `@axonpack/expo-devtools` alone.
...
### Docs site (run from `apps/docs`)

- `bun run dev` — `next dev`. `turbo run dev --filter=axonpack-docs` from the root is the same thing.
- `bun run build` — `next build` with `output: 'export'`, writing a plain directory of files to `out/`. Prerenders every page plus an OG image, an `llms.txt`, a `content.md` and the whole search index per page. `bun run start` serves `out/` so the export itself can be checked, not just the dev server.
- `bun run lint` / `bun run check-types` — `oxlint src` / `next typegen && tsc --noEmit`.
- **One folder per package, in every structure that holds anything package-specific**: `content/docs/<slug>/` for its pages, `public/<slug>/` for its assets, `/docs/<slug>/` for its routes, one entry in `src/lib/packages.ts`. This site documents several independent libraries, not one product, so the package is the top-level axis and each is a `"root": true` folder — which is what turns it into a tab in the sidebar's package switcher. `content/docs/overview/` is the only tab that is not a package. Adding a library is spelled out in `apps/docs/README.md`; the short version is one entry in `packages.ts` plus one folder in each of the paths above.
- **A page not listed in its folder's `meta.json` does not appear**, the same trap as Expo's own `constants/navigation.js`.
- **Search has no server.** `src/app/api/search.json/route.ts` exports fumadocs' `staticGET`, which serialises the whole index into `out/api/search.json` at build time; the client fetches it on the first query and answers locally. The route is `search.json` and not `search` because GitHub Pages types and compresses by file extension — extensionless it ships as uncompressed `application/octet-stream`, roughly five times the bytes. Every entry is tagged with `page.slugs[0]` — its root folder, so its package — and `src/components/search.tsx` reads the current package out of the pathname and opens search filtered to it, with a footer row of tags to switch package or clear the filter. That scopes _results_, not the download: one index still covers every package and the whole file is fetched. Raising that ceiling means one index file per package, or a hosted index; both stay static and only `search.tsx` changes. **Do not un-memoize the search client** — `useDocsSearch` keys its effect on `client.deps`, which is a fresh array on every `staticClient()` call, so an unmemoized client re-queries on every render.
- A **planned** package gets a `packages.ts` entry with `status: 'planned'` and a section on the roadmap page, and no tab — the same rule `notes/plan.md` already applies to itself.
- `/docs` is not an MDX page: `src/app/docs/page.tsx` is a real prerendered page that bounces to `/docs/overview`. A static export has no server, so `redirects()` in `next.config.mjs` would do nothing. That page is also why the catch-all is `[...slug]` and not `[[...slug]]` — the optional form collides with a sibling `page.tsx`.
- `next.config.mjs` sets `turbopack.root` to the **repo root**, not to `apps/docs`. bun hoists `next` above the app directory, and Turbopack refuses to compile anything outside its root — without this the build fails with `Could not find the Next.js package`.
- **GitHub Pages at the organisation root**, `https://axonpack.github.io`, via `.github/workflows/docs.yml`: builds on every PR touching `apps/docs/**`, and on push to `main` force-pushes `apps/docs/out` to the separate repo `axonpack/axonpack.github.io` over a deploy key (`PAGES_DEPLOY_KEY`). Only a repo named exactly `<org>.github.io` is served from the root — this one could only ever be `axonpack.github.io/axonpack` — which is the whole reason for the second repo. **That repo holds build output and nothing else**; each deploy replaces its history, so anything hand-written there is discarded. A root site has no prefix, so `NEXT_PUBLIC_BASE_PATH` is unset; it stays wired up because a move to a subpath has to reach two places and only one is automatic — `basePath` in `next.config.mjs`, and `src/components/search.tsx`, which builds the search-index URL by hand because fumadocs reads its base path from `import.meta.env.BASE_URL`, a Vite convention Next does not set. Both variables are declared in `turbo.json`'s `build.env` so a cached build is never reused across prefixes. `public/.nojekyll` has to stay, or Pages' Jekyll pass drops `_next`. Setup steps for the second repo and its deploy key are in `apps/docs/README.md`.
- `apps/docs/.prettierrc` exists for the same reason `packages/@axonpack/expo-devtools/.prettierrc` does: without it the root `format` task applies Prettier's defaults and rewrites the whole app to double quotes at 80 columns. The root glob (`**/*.{ts,tsx,md}`) does not reach `.mdx` at all, so page content is never reformatted.
- Writing conventions, and where each thing lives, are in `apps/docs/README.md`.
...
- Root `turbo.json`'s `outputs: [".next/**", ..., "out/**"]` came from the Next.js starter. It is now correct for `apps/docs` and still doesn't match `@axonpack/expo-devtools`'s actual build output (`build/**`) — `turbo run build` warns `no output files found for task @axonpack/expo-devtools#build` and can never cache that one. Not a bug to "fix" reflexively; just don't be surprised the package build always re-runs.
