...
- `icon` is top-level and must be passed explicitly — there is no way to detect it. A device's installed launcher icon isn't reachable from JS without native code, and `expoConfig.icon` is a build-time path string, not something `Image` can load in a standalone build. It reaches the header through `stores/app-identity.store.ts` rather than a prop, because `DevtoolsOverlay` is mounted somewhere else in the tree and never sees the config object. There was a `name` alongside it; the panel header is now a single row holding the app icon, the (scrollable) tab bar and the close button, which left no room for a title, so the option was removed rather than kept as a no-op.
