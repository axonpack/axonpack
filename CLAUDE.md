...
- **GitHub Pages**, via `.github/workflows/docs.yml`: builds on every PR touching `apps/docs/**`, deploys on push to `main`. It is a project site, so everything is served under `/axonpack` and `NEXT_PUBLIC_BASE_PATH` carries that prefix. Two places consume it and only one is automatic — `basePath` in `next.config.mjs`, and `src/components/search.tsx`, which builds the search-index URL by hand because fumadocs reads its base path from `import.meta.env.BASE_URL`, a Vite convention Next does not set. `NEXT_PUBLIC_BASE_PATH`/`NEXT_PUBLIC_SITE_URL` are declared in `turbo.json`'s `build.env` so a cached build is never reused across base paths. Moving to a custom domain means dropping both variables and adding `public/CNAME`; `public/.nojekyll` has to stay either way, or Pages' Jekyll pass drops `_next`.
