...
- 3b04c5d: - **Navigation tab**: every screen the app moved through, with when, from where, the params and how long it stayed on top
...
- e34ba2a: - **An Axonpack tab in React Native DevTools**, beside Console and Sources, from the one line in `metro.config.js` you already had
...
- Updated dependencies [99f49a5]
  - @axonpack/react-native-devtools-tab@0.1.1

## 3.0.0

### Major Changes

- 7f3287f: The whole list of what moved where is in
...
- 3922335: - **`expo-clipboard` is now a peer dependency**, so it resolves to the version your own Expo SDK ships. As a direct dependency it was pinned to `~57.0.1`, which tied this package to SDK 57 — that was the only thing that did
...
- f8adf85: - **Add a key** to a store from the Storage tab — you pick the name, the type and the value, and a key that already exists is refused
...
- 038b71d: - New **Crashes** tab: every crash the app has hit, with the unread count on the tab itself
...
- 038b71d: - New **Debug** tab for the tools that break the app on purpose — blocking a thread, and crashing one
...
- 1f5e7d8: - New **Storage** tab: every key in every store you register, with its value, type and byte size, plus the store's total size and largest key
...
- f8f2fa0: - Search the Payload, Preview and Response tabs of a request, with every match highlighted in place — in the JSON tree, in syntax-coloured code, and in the raw body
