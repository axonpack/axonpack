...
describe('flattenNavigator', () => {
  ...
  it('runs a rail through the routes, ending at the last, and carries a parent rail past a nested block', () => {
    ...
    expect(rows.map((row) => row.rail)).toEqual(['through', 'end', 'through', 'end']);
    ...
    expect(nested?.trail).toEqual([true]);
  });
  ...
  it('hangs a container mounted in a screen under that screen, which then hosts rather than shows', () => {
    const root = {
      type: 'stack',
      index: 1,
      routes: [
    ...
    expect(summary(flattenNavigator(root, 'checkout-1', hosted))).toEqual([
      { depth: 0, kind: 'route', label: 'Home', active: false, onScreen: false },
      { depth: 0, kind: 'route', label: 'Checkout', active: true, onScreen: false },
