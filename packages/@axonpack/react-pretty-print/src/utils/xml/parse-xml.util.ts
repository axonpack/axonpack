 ...
 *
 * React Native has no `DOMParser` and no DOM, so handing `documentElement` to a renderer is not an
 * option. This is the smallest parser that answers what a reader asks of a document: what are the
 * elements, what are their attributes, and what text is inside them.
 *
 * It is deliberately not a validator. A document is read to be understood, not to be certified, so
 * one that does not parse returns an error for the caller to show its raw text against, rather than
 * a thrown exception.
 */
...
export function parseXml(source: string): XmlParseResult {
  // The prologue, comments and doctype are dropped rather than represented: they are rarely what
  // someone opened a document to read, and the caller still has the raw text.
  const text = source
    .replace(/^﻿/, '')
    .replace(/<\?[\s\S]*?\?>/g, '')
    .replace(/<!--[\s\S]*?-->/g, '')
