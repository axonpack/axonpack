...
/**
 * `pattern` is null when the query could not be compiled — callers then filter and highlight
 * nothing.
 *
 * Plain data on purpose. A caller that already has a search matcher of this shape passes it
 * straight in: TypeScript matches structurally, so nothing has to import this type or convert to
 * it. That is why the renderers take a matcher rather than a callback.
 */
export type Matcher = {
  pattern: RegExp | null;
  invalid: boolean;
...
/**
 * Compile once per query, never per row: a caller filtering a list runs the result over every
 * visible entry on every keystroke, and compiling there would be the slowest thing on screen.
 */
