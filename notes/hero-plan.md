...
- **Agent claim is the export, not an integration.** Schema-versioned JSON and copy-as-Markdown are
  real today. An MCP server is not. Do not imply one until it ships.
- **Animation:** the phone beside the copy. `public/assets/network-*.webp` and the `showcase` copy
  already exist and nothing renders them. Static first, CSS only, no aurora.
